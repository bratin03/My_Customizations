...
extract_and_copy() {
    ...
    # Call unzip_7z function
    unzip_7z "$filename" "$password"
    
    if [[ $? -eq 0 ]]; then
        # Find the first extracted .txt file
        local txt_file=$(find . -type f -name "*.txt" | head -n 1)
        
        if [[ -n "$txt_file" ]]; then
            cp "$txt_file" input.txt
            echo "Copied $txt_file to input.txt"
        else
            echo "No .txt file found after extraction."
            return 1
        fi
    fi
